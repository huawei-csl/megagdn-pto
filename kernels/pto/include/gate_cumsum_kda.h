...
// UbND alias — identical to chunk_cumsum.h.
...
AICORE void gate_cumsum_kda_kernel(__gm__ half *g_ptr, __gm__ float *g_sum_ptr,
  ...
  // Per-tile UB layout:
  //   [0           .. HalfBlockBytes)        = g input,  fp16 staging
  //   (ChunkSize × CTC) [GUbAddr     .. +BlockBytes)           = g input,  fp32
  //   (ChunkSize × CTC) [SUbAddr     .. +BlockBytes)           = g_sum out,
  //   fp32      (ChunkSize × CTC) [AccUbAddr   .. +CTC*4)                = row
  //   accumulator fp32 (1 × CTC)
  ...
  using GmFloat =
      GlobalTensor<float, GmShape, GmStride>;  // output g_sum (fp32)
