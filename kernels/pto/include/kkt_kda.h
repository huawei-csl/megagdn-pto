...
AICORE void kkt_kda_kernel(__gm__ half *k_ptr, __gm__ float *g_cs_ptr,
                           ...
                           int64_t total_tokens, int32_t num_heads,
                           uint64_t ffts_addr) {
  ...
  using GmHalfWsOut =
      GlobalTensor<half, GmShapeDyn, Stride<1, 1, 1, ChunkSize, 1>>;
  ...
  constexpr int32_t BETA_ADDR =
      COLH_ADDR + HalfChunk * 16 * 2;  // [1, HalfChunkF] fp32 (beta)
  constexpr int32_t BETAH_ADDR =
      BETA_ADDR + HalfChunkF * 4;  // [1, HalfChunkH] fp16 (beta staging)
  constexpr int32_t MSKC_ADDR =
      BETAH_ADDR + HalfChunkH * 2;  // [HalfChunk, 16] fp32 (mask col)
