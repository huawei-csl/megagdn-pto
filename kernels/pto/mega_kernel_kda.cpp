...
// in sequence with SyncAllImpl<false>() barriers between stages.
//
// Stages:
...
// Full Cube+Vec all-core barrier.  Uses FFTS flags 11-14, distinct from the
// sub-kernels' internal sync flags (6-9), so a sub-kernel's exit-sync
// immediately followed by this barrier never reuses the same flag back-to-back.
...
AICORE void mega_permute_THK_to_HTK(__gm__ T* src, __gm__ T* dst, int64_t T_len,
                                    int32_t HV) {
