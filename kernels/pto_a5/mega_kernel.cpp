...
AICORE inline void SyncAllImpl()
{
    ...
    wait_flag_dev(PIPE_S, SYNC_AIV_FLAG);
    ...
    wait_flag_dev(PIPE_S, SYNC_AIC_AIV_FLAG);
#endif
}
    ...
    __gm__ T *src, __gm__ T *dst, int64_t T_len)
{
    ...
    for (int64_t bi = static_cast<int64_t>(cid); bi < num_tok_blocks;
         bi += static_cast<int64_t>(num_blocks)) {
        int64_t t0 = bi * BLOCK;
    ...
    uint32_t num_matrices, int64_t total_tokens)
{
    ...
    for (uint32_t m = cid; m < num_matrices; m += num_blocks) {
