...
AICORE void cumsum_kernel(
    ...
    uint64_t ffts_addr)
{
  // get_block_idx(): Returns this AI core's index (0..num_blocks-1).
...
// Using these guards lets us put Vec, Cube, and host code in one file.
  ...
  if (cu_seqlens == nullptr) {
    ...
    // Core `cid` handles chunks cid, cid+num_blocks, cid+2*num_blocks, ...
    ...
    for (int64_t gi = static_cast<int64_t>(cid); gi < total_chunks;
         gi += static_cast<int64_t>(num_blocks)) {
      ...
      TMOV(acc_ub, g_row_0);
      // pipe_barrier(PIPE_ALL): Ensures all pending Vec (SIMD) operations complete
