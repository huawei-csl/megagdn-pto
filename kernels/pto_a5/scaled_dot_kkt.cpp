...
//   pipe_barrier(PIPE_ALL)    — Local Vec barrier (ensure all Vec ops complete)
    ...
    uint64_t ffts_addr)
{
  ...
  auto num_blocks = get_block_num();  // Total AI cores launched (like CUDA gridDim.x)
  ...
  for (int64_t work_idx = 0;
       work_idx < (total_work + num_blocks - 1) / num_blocks; ++work_idx) {
    int64_t pid = work_idx * static_cast<int64_t>(num_blocks) +
    ...
    for (int64_t ci = 0; ci < num_chunks; ++ci) {
      ...
      //
      // The receiving side calls wait_flag_dev(PIPE_S, flag_id) to wait for this signal.
      //
      // In this kernel:
      //   Cube sets flag 0/1 → Vec waits on wait_flag_dev(PIPE_S, 0/1) (KK^T ready)
      //   Vec sets flag 2/3 → Cube waits on wait_flag_dev(PIPE_S, 2/3) (workspace free)
      //
  ...
  // Without this, Cube would hang on wait_flag_dev(PIPE_S, 2/3) at the first iteration.
  ...
  for (int64_t work_idx = 0;
    ...
    for (int64_t ci = 0; ci < num_chunks; ++ci) {
      ...
      if (local_valid > 0) {
        ...
        pipe_barrier(PIPE_ALL);       // Wait for TMOV to complete
...

        {
          ...
          GlobalTensor<half, decltype(_gs), pto::Stride<1, 1, 1, NumHeads * ChunkSize, 1>> _gm(A_handle + a_gm_offset, _gs);
      ...
      // Cube is waiting on wait_flag_dev(PIPE_S, 2+slot) before writing the next chunk.
