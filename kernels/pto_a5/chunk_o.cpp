...
//   wait_flag_dev(PIPE_S, flag)     — wait for cross-core signal
// ============================================================================
    ...
    uint64_t ffts_addr)
{
  ...
  // cid = which AI core am I? (0..num_blocks-1). Used to partition work items.
  auto cid = get_block_idx();
  // num_blocks = total number of AI cores running this kernel in parallel.
  auto num_blocks = get_block_num();
  ...
  // Each AI core (cid) picks every num_blocks-th work item (round-robin).
...
// =====================================================================
  ...
  if (cu_seqlens == nullptr) {
    ...
    for (int64_t work_idx = static_cast<int64_t>(cid);
         work_idx < total_work;
         work_idx += static_cast<int64_t>(num_blocks)) {
      // Wait for Vec to finish with previous chunk's workspace (flag 3)
      if (!first_cube_iter) wait_flag_dev(PIPE_S, 3);
      ...
      // downstream GEMMs see a clean C×D matrix.
      {
        ...
        GlobalTensor<half, decltype(_gs), pto::Stride<1, 1, 1, BSND_QK_STRIDE, 1>> _gm(Q_handle + qk_off, _gs);
        TLOAD(_l1, _gm);
      ...
      // ── Load K [valid_rows × D] from GM → L1 ────────────────────────
      {
        ...
        Shape<1, 1, 1, DYNAMIC, DYNAMIC> _gs;
        _gs.shape[3] = valid_rows; _gs.shape[4] = HiddenSize;
        GlobalTensor<half, decltype(_gs), pto::Stride<1, 1, 1, BSND_QK_STRIDE, 1>> _gm(K_handle + qk_off, _gs);
        TLOAD(_l1, _gm);
        if (valid_rows != ChunkSize) TFILLPAD(_l1, _l1);
      }
      ...
      // ── Load S [D × D] from GM → L1  (accumulated hidden state) ─────
      {
        ...
        GlobalTensor<half, decltype(_gs), pto::Stride<1, 1, 1, HiddenSize, 1>> _gm(S_handle + s_offset, _gs);
        TLOAD(_l1, _gm);
      }
      ...
      // ── Store QS [C × D] from L0C → GM workspace ────────────────────
      {
        ...
        GlobalTensor<half, decltype(_gs), pto::Stride<1, 1, 1, HiddenSize, 1>> _gm(
      ...
      // ── Load V [valid_rows × D] from GM → L1 ────────────────────────
      {
        ...
        Shape<1, 1, 1, DYNAMIC, DYNAMIC> _gs;
        _gs.shape[3] = valid_rows; _gs.shape[4] = HiddenSize;
        GlobalTensor<half, decltype(_gs), pto::Stride<1, 1, 1, BSND_V_STRIDE, 1>> _gm(V_handle + v_off, _gs);
        TLOAD(_l1, _gm);
        if (valid_rows != ChunkSize) TFILLPAD(_l1, _l1);
      }
    ...
    for (int64_t si = 0; si < num_seqs; ++si) {
      ...
      for (int64_t ci = 0; ci < nc; ++ci) {
        for (int32_t h = 0; h < NumHeads; ++h) {
          if (gi % static_cast<int64_t>(num_blocks) ==
              static_cast<int64_t>(cid)) {
            if (!first_cube_iter_v) wait_flag_dev(PIPE_S, 3);
            set_flag(PIPE_FIX, PIPE_M, EVENT_ID0);
            wait_flag(PIPE_FIX, PIPE_M, EVENT_ID0);
  ...
  if (cu_seqlens == nullptr) {
    ...
    for (int64_t work_idx = static_cast<int64_t>(cid);
...

      {
        Shape<1, 1, 1, DYNAMIC, DYNAMIC> _gs;
        _gs.shape[3] = local_rows; _gs.shape[4] = HiddenSize;
        GlobalTensor<half, decltype(_gs), pto::Stride<1, 1, 1, BSND_V_STRIDE, 1>> _gm(
